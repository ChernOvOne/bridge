...
### Первая ENTRY-нода новой независимой инсталляции

Если это **новый набор** мостов (не подключаешься к уже работающему):

```
br → [8] Bridge credentials → [5] Сгенерировать новые
```

CLI сам сгенерит UUID, Reality-keypair (через `docker run xray x25519`) и shortId, спросит SNI/dest/port с дефолтами (`max.ru` / `max.ru:443` / `7443`) и сохранит всё в `state.json`. Дальше — `[1] Управление exit-нодами → [1] Добавить`: установочные команды на EXIT-ноды подхватят эти credentials автоматически.

> Reality dest должен поддерживать TLS 1.3. Проверь: `openssl s_client -connect <dest>:443 -tls1_3`.
...
[8]  Bridge credentials                   ← [1] импорт b64 · [2] вручную · [3] экспорт · [4] удалить · [5] сгенерировать новые
[9]  Обновить bridge-cli из GitHub
[0]  Выход
```
